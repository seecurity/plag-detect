...
//		ArrayList<MatchVals> tiles = new ArrayList<MatchVals>();
		if (mML < 1)
			System.err
		...
		// Compute Tiles
		ArrayList<MatchVals> tiles = RKR_GST(s1, s2, mML, 20);
		...
		for(MatchVals localTiles:result.getTiles()){
			System.out.print("("+localTiles.patternPostion+",");
			System.out.print(localTiles.textPosition+",");
			System.out.print(localTiles.length+")");
		}
	 ...
	 * @param T
	 *            String
	 * @param minimalMatchingLength
	 *            Matching length value
	 * @param initsearchSize
	 *            search size
	 * @return tiles
	 */
	public static ArrayList<MatchVals> RKR_GST(String P, String T,
		...
		return RKR_GST(PList, TList, minimalMatchingLength, initsearchSize);
	}
	...
	public static ArrayList<MatchVals> RKR_GST(String[] PList, String[] TList,
			int minimalMatchingLength, int initsearchSize) {
		ArrayList<Queue<MatchVals>> matchList = new ArrayList<>();
		ArrayList<MatchVals> tiles = new ArrayList<>();
		...
		while (!stop) {
			...
			int Lmax = scanpattern(matchList, s, PList, TList);
			// if very long string no tiles marked. Iterate with larger s
			if (Lmax > 2 * s)
				s = Lmax;
			else {
				markStrings(tiles,matchList, s, PList, TList);
	 ...
	 */
	public static int scanpattern(ArrayList<Queue<MatchVals>> matchList, int s, String[] P, String[] T) {

		int longestMaxMatch = 0;
		Queue<MatchVals> queue = new LinkedList<>();
	...
	private static void markStrings( ArrayList<MatchVals> tiles, ArrayList<Queue<MatchVals>> matchList, int s, String[] P, String[] T) {
	...
	private static Object jumpToNextUnmarkedTokenAfterTile(int pos, String[] stringList) {
        ...
        /**
         * Demonstration of the function call
         * @param s 
         */
